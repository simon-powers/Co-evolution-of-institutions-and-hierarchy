 ...
 * @author spowers
 */
public class SimulationMultipleReps {
    ...
    public static void main(String[] args) {
    	...
    	double coopB=0.9;
    	double coopC=0.1;
    	double initPropCoop=0;
    	double initPropSelfish=1;
    	double initHvalue=0;
    	...
    	if(NUM_DEMES==1)
    		MIG_RATE = 0;
    	int every_t=300; //print stats every t generations
    	try{
    		String filename = "Reps10ParamsNoEvolveAlphaxThr" + xThr + "instCost" + inst_cost + ".csv";
    		FileWriter write = new FileWriter(filename);
    		PrintWriter printer = new PrintWriter(write);
    		printer.println("coop,defect,asocial,consensusP,meanPTrait,skew,"
    				+ "instCost,meanAlpha,meanSocialK");
    		for(int rep=0; rep<NUM_REPS; rep++){
    			System.out.println(rep);
    			Model model = new Model(NUM_DEMES, T, k_base,
    					inst_cost, MUT_RATE,
    					MIG_RATE, beta, grad, coopB, coopC,
    					numListeners, kScale, xThr, H_MUT_RATE, every_t,
    					evolveAlpha);
    			Results results = model.runFunc();
    			printer.println(results.meanFreqCoop() + "," +
    					results.meanFreqSelfish() + "," +
    					results.meanFreqLoner() + "," +
    					(1 - results.meanHByConsensus()) + "," +
    					(1 - results.meanH()) + "," +
    					results.meanAlphaSkew() + "," +
    					results.meanInstCost() + "," +
    					results.meanAlpha() + "," +
    					results.meanGroupK());
